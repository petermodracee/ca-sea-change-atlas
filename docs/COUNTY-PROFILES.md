...
- **SLR** uses NOAA's ocean-connected inundation (`_slr_` tables) at 2, 4, 6, 8 and 10 ft above MHHW, and separately connected plus NOAA's unconnected low-lying areas (`_low_` tables). Both series are written to the snapshot (`counts` and `countsWithLow`; see [Sea level rise views](#sea-level-rise-connected-and-low-lying-views)). The pipeline asserts the extents nest (nothing inundated at 2 ft is dry at 4 ft), which the validator's non-decreasing rule also enforces, and the validator requires `countsWithLow` never to fall below `counts`.
...
### Sea level rise connected and low-lying views

NOAA maps ocean-connected inundation and hydrologically unconnected low-lying areas separately, and both are exposure. Each SLR increment section (People at Risk, Flooded Facilities, Jobs) therefore carries two series, `counts` (connected) and `countsWithLow` (connected plus low-lying), and the deck shows a **Connected only / Including low-lying areas / Both** toggle at the top of each, default Both. The state is shared by the three sections and kept in the URL as `?low=conn|withlow|both`, so a shared link reproduces the view.

How it is built: every mark for every view is rendered at build time and tagged `data-cpd-show="conn"`, `"withlow"` or `"both"` (a space-separated list of the views it belongs to); a section's `data-cpd-mode` picks the view and one CSS rule (`[data-cpd-moded][data-cpd-mode=…] [data-cpd-show]:not([data-cpd-show~=…])`) hides the rest. `site/js/county-deck.js` only sets the mode, keeps the toggles in step and updates the URL. In Both, the connected mark keeps the solid ramp colour and the additional low-lying exposure is a lighter dashed-edged segment stacked on it (facilities: a third segment before the unexposed remainder), with a legend item "Additional area affected by groundwater rise and isolated low-lying flooding"; the end-of-bar label is the connected-plus-low-lying total and the tooltip and data table give both. The chart axis is fixed to the larger series so switching views never rescales it. Each section also has three callouts, one per view, and a footnote defining "low-lying" with sources (NOAA's mapping method for the definition, Befus et al. 2020 for groundwater rise). A section whose data has no `countsWithLow` (any fixture) shows no toggle and behaves as before. Jobs at Risk has no chart, so its toggle switches its callout. The reasoning and the sourcing caveat are in [`DECISIONS.md`](DECISIONS.md#slr-sections-show-ocean-connected-and-low-lying-exposure-both-by-default).
