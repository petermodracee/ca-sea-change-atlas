...
How it is drawn: in People at Risk each column is the combined count, the ocean-connected part solid in its ramp colour and the additional low-lying part a lighter dashed-edged segment stacked on it; in Flooded Facilities each bar is the exposed part, then the additional low-lying part (dashed), then the unexposed remainder. The end-of-bar label is the combined total, the legend has an item "Additional area affected by groundwater rise and isolated low-lying flooding", the callout and prose state one number ("… including isolated low-lying areas"), and the data table lists both parts. The chart axis fits the combined values. Each section has a one-line footnote glossing "low-lying" with short links to NOAA's mapping method and Befus et al. 2020 (the full explanation is pending a sitewide methodology page; none exists yet). Every segment of a Flooded Facilities bar carries the same tooltip, one category per line (ocean-connected, additional low-lying, not yet exposed). Jobs at Risk has no chart, so it is a single callout. A section whose data has no `countsWithLow` (any fixture) draws connected-only figures with no gloss, as before. Total economy's "Jobs under 6 ft of sea level rise" stat pair uses the same combined series, with its own one-line footnote.
